...
import com.company.oop.taskManagementSystem.commands.changeCommands.ChangePriorityCommand;
...
public class ChangePriorityCommandTests {
    ...
    @Test
    public void should_ChangeStoryPriority_When_PassedValidInput() {

        String oldPriority = story.getPriority().toString();

        List<String> parameters = List.of(storyTitle, Priority.MEDIUM.toString());

        String successMessage = command.execute(parameters);

        Assertions.assertEquals(Priority.MEDIUM, repository.getTeams().get(0).getBoards().get(0).getStories().get(0).getPriority());
        Assertions.assertEquals(successMessage, String.format(ChangePriorityCommand.PRIORITY_SET_SUCCESSFULLY, story.getType(), storyTitle, oldPriority, Priority.MEDIUM));
    }

    @Test
    public void should_ChangeBugPriority_When_PassedValidInput() {

        String oldPriority = bug.getPriority().toString();

        List<String> parameters = List.of(bugTitle, Priority.HIGH.toString());

        String successMessage = command.execute(parameters);

        Assertions.assertEquals(Priority.HIGH, repository.getTeams().get(0).getBoards().get(0).getBugs().get(0).getPriority());
        Assertions.assertEquals(successMessage, String.format(ChangePriorityCommand.PRIORITY_SET_SUCCESSFULLY, bug.getType(), bugTitle, oldPriority, Priority.HIGH));
    }
    ...
    public void should_ThrowException_When_StoryNotExist() {
        List<String> parameters = List.of("SomeOtherStoryName", Priority.HIGH.toString());
        ...
        String expectedErrorMessage = format(ChangePriorityCommand.NO_TASK_IN_TEAM_ERR_MESSAGE, "SomeOtherStoryName", team.getName());
    ...
    public void should_ThrowException_When_BugNotExist() {
        List<String> parameters = List.of("SomeOtherBugName", Priority.HIGH.toString());
    ...
    //Bug and Story have priority -> High, Medium, or Low.
